    ...
    describe('post_forgot_password should render the ForgotPassword page with account details', () => {
        describe('email and security code body fields should match with database entry', () => {
            ...
            test('account is a user', async () => {
                ...
                expect(find_one_mock).toHaveBeenCalledWith(User, {email: 'user_email', security_code: '1111'}, 'id_number email security_code');
                expect(find_one_mock).toHaveBeenCalledWith(Admin, {email: 'user_email', security_code: '1111'}, 'id_number email security_code');
                expect(find_one_mock).toHaveBeenCalledWith(Driver, {email: 'user_email', security_code: '1111'}, 'id_number email security_code');
            ...
            test('account is an admin', async () => {
                ...
                expect(find_one_mock).toHaveBeenCalledWith(User, {email: 'admin_email', security_code: '1111'}, 'id_number email security_code');
                expect(find_one_mock).toHaveBeenCalledWith(Admin, {email: 'admin_email', security_code: '1111'}, 'id_number email security_code');
                expect(find_one_mock).toHaveBeenCalledWith(Driver, {email: 'admin_email', security_code: '1111'}, 'id_number email security_code');
            ...
            test('account is a driver', async () => {
                ...
                expect(find_one_mock).toHaveBeenCalledWith(User, {email: 'driver_email', security_code: '1111'}, 'id_number email security_code');
                expect(find_one_mock).toHaveBeenCalledWith(Admin, {email: 'driver_email', security_code: '1111'}, 'id_number email security_code');
                expect(find_one_mock).toHaveBeenCalledWith(Driver, {email: 'driver_email', security_code: '1111'}, 'id_number email security_code');
        ...
        describe('email and security code body fields do not match with database entry', () => {
            ...
            test('account is not a user, admin or driver', async () => {
                ...
                expect(find_one_mock).toHaveBeenCalledWith(User, {email: 'email', security_code: '1111'}, 'id_number email security_code');
                expect(find_one_mock).toHaveBeenCalledWith(Admin, {email: 'email', security_code: '1111'}, 'id_number email security_code');
                expect(find_one_mock).toHaveBeenCalledWith(Driver, {email: 'email', security_code: '1111'}, 'id_number email security_code');
...
describe('ForgotPassController - post_change_F_password', () => {
    describe('new_password and retype_password body fields should match', () => {
        ...
        test('account is a user', async () => {
            let req = {body: {user_new_password: 'password', user_retype_password: 'password', id_number: 9999999}};
            let res = {render: jest.fn()};
            ...
            expect(find_one_mock).toHaveBeenCalledWith(User, {id_number: 9999999}, 'id_number password');
            expect(find_one_mock).toHaveBeenCalledWith(Admin, {id_number: 9999999}, 'id_number password');
            expect(find_one_mock).toHaveBeenCalledWith(Driver, {id_number: 9999999}, 'id_number password');

            expect(hash_mock).toHaveBeenCalledWith('password', 10);
            expect(update_one_mock).toHaveBeenCalledWith(User, {id_number: 9999999}, {password: 'hashed_password'});
        ...
        test('account is an admin', async () => {
            ...
            expect(hash_mock).toHaveBeenCalledWith('password', 10);
            expect(update_one_mock).toHaveBeenCalledWith(Admin, {id_number: 9999999}, {password: 'hashed_password'});
            expect(res.render).toHaveBeenCalledWith('Login', {code_change: true});
        });
        ...
        test('account is a driver', async () => {
            ...
            const find_one_mock = jest.spyOn(db, 'find_one').mockResolvedValueOnce(null).mockResolvedValueOnce(null).mockResolvedValueOnce({id_number: 9999999, password: 'password'});
            const update_one_mock = jest.spyOn(db, 'update_one').mockResolvedValueOnce();
            const hash_mock = jest.spyOn(bcrypt, 'hash').mockResolvedValueOnce('hashed_password');
