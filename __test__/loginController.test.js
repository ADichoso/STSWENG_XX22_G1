...
describe('loginController', () => {
  describe('get_login', () => {
    ...
    test('should redirect to user profile if user is found in user collection', async () => {
      const req = { session: { user_id_number: '12196290' } };
      ...
      expect(find_one_mock).toHaveBeenCalledWith(user, { user_id_number: '12196290' }, { user_id_number: 1 });
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.redirect).toHaveBeenCalledWith('/Profile?user_id_number=12196290');
    });

    test('should redirect to admin profile if user is found in admin collection', async () => {
      const req = { session: { user_id_number: '123456789' } };
      const res = { status: jest.fn().mockReturnThis(), redirect: jest.fn() };
      const find_one_mock = jest.spyOn(db, 'find_one').mockResolvedValueOnce(false).mockResolvedValueOnce(true);

      await loginController.get_login(req, res);

      expect(find_one_mock).toHaveBeenCalledWith(user, { user_id_number: '123456789' }, { user_id_number: 1 });
      expect(find_one_mock).toHaveBeenCalledWith(admin, { user_id_number: '123456789' }, { user_id_number: 1 });
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.redirect).toHaveBeenCalledWith('/ProfileAdmin?user_id_number=123456789');
    });
    ...
    test('should redirect to driver profile if user is found in driver collection', async () => {
      ...
      expect(find_one_mock).toHaveBeenCalledWith(user, { user_id_number: '123456789' }, { user_id_number: 1 });
      expect(find_one_mock).toHaveBeenCalledWith(admin, { user_id_number: '123456789' }, { user_id_number: 1 });
      expect(find_one_mock).toHaveBeenCalledWith(driver, { user_id_number: '123456789' }, { user_id_number: 1 });
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.redirect).toHaveBeenCalledWith('/ProfileDriver?user_id_number=123456789');
    });
  ...
  describe('post_login', () => {
    test('should redirect to SecurityCheck page if login is successful', async () => {
      ...
      const find_one_mock = jest.spyOn(db, 'find_one').mockResolvedValueOnce({ user_id_number: '123456789', password: 'hashed' });
      bcrypt.compare.mockResolvedValueOnce(true);

      await loginController.post_login(req, res);

      expect(find_one_mock).toHaveBeenCalledWith(user, { user_id_number: '123456789' }, { user_id_number: 1, password: 1 });
      expect(bcrypt.compare).toHaveBeenCalledWith('password', 'hashed');
      expect(req.session.user_id_number).toBe('123456789');
      expect(res.redirect).toHaveBeenCalledWith('/SecurityCheck?user_id_number=123456789');
    });

    test('should redirect to SecurityCheck page if admin login is successful', async () => {
      const req = { body: { user_id_number: '123456789', user_password: 'password' }, session: {} };
      const res = { redirect: jest.fn() };
      const find_one_mock = jest.spyOn(db, 'find_one').mockResolvedValueOnce(false).mockResolvedValueOnce({ user_id_number: '123456789', password: 'hashedPassword' });
      bcrypt.compare.mockResolvedValueOnce(true);

      await loginController.post_login(req, res);

      expect(find_one_mock).toHaveBeenCalledWith(user, { user_id_number: '123456789' }, { user_id_number: 1, password: 1 });
      expect(find_one_mock).toHaveBeenCalledWith(admin, { user_id_number: '123456789' }, { user_id_number: 1, password: 1 });
      expect(bcrypt.compare).toHaveBeenCalledWith('password', 'hashed');
      expect(req.session.user_id_number).toBe('123456789');
      expect(res.redirect).toHaveBeenCalledWith('/SecurityCheck?user_id_number=123456789');
    });
    ...
    test('should redirect to SecurityCheck page if driver login is successful', async () => {
      const req = { body: { user_id_number: '123456789', user_password: 'password' }, session: {} };
      const res = { redirect: jest.fn() };
      const find_one_mock = jest.spyOn(db, 'find_one').mockResolvedValueOnce(false).mockResolvedValueOnce(false).mockResolvedValueOnce({ user_id_number: '123456789', password: 'hashedPassword' });
      bcrypt.compare.mockResolvedValueOnce(true);
      ...
      expect(find_one_mock).toHaveBeenCalledWith(user, { user_id_number: '123456789' }, { user_id_number: 1, password: 1 });
      expect(find_one_mock).toHaveBeenCalledWith(admin, { user_id_number: '123456789' }, { user_id_number: 1, password: 1 });
      expect(find_one_mock).toHaveBeenCalledWith(driver, { user_id_number: '123456789' }, { user_id_number: 1, password: 1 });
      expect(bcrypt.compare).toHaveBeenCalledWith('password', 'hashed');
      expect(req.session.user_id_number).toBe('123456789');
      expect(res.redirect).toHaveBeenCalledWith('/SecurityCheck?user_id_number=123456789');
    });
